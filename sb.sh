...
show_banner() {
    ...
    IPV6_ADDRESS=$(curl -s -6 ifconfig.me 2>/dev/null || echo "未检测到IPv6")
    ...
    echo -e "  内核版本: $(uname -r)"
    echo -e "  架构: $(uname -m)"
    echo -e "  公网IPv4: ${IPV4_ADDRESS}"
    echo -e "  公网IPv6: ${IPV6_ADDRESS}"
...
show_main_menu() {
    ...
    echo -e "${WHITE}1)${NC} ${GREEN}安装环境${NC} - 安装基础环境和工具"
...
show_config() {
    ...
    echo -e "  内网IP: $(hostname -I | awk '{print $1}')"
    echo -e "  时区: $(timedatectl | grep 'Time zone' | awk '{print $3}')"
    ...
    if [ -f "/root/cert.crt" ]; then
        ...
        echo -e "  证书域名: $(openssl x509 -noout -subject -in /root/cert.crt | grep -oP 'CN = \K[^ ,]+' 2>/dev/null || echo '未找到域名')"
        ...
        echo -e "  证书域名: $(openssl x509 -noout -subject -in /etc/ssl/certs/cert.crt | grep -oP 'CN = \K[^ ,]+' 2>/dev/null || echo '未找到域名')"
    ...
    echo -e "  Firewall: $(systemctl is-active ufw 2>/dev/null || echo 'inactive')"
